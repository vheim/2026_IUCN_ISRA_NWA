...
### Script:     ~~2026_IUCN_ISRA_NWA/R/Rscript6x_Visualise_Move_Persistence_Tracks.R
### Author:     Vital Heim
### Version:    1.0
### ====================================================================================================
...
### such as animal tracking data. Here, we modify the script to visualise the outputs for a move
### persistence model.
### 
...
select_and_read_file <- function(folder_path, pattern = "\\.rds$") {
  ...
  if (grepl("fitted", selected_file, ignore.case = TRUE)) {
    ...
    tracktype <<- "predicted"
  } else if (grepl("rerouted", selected_file, ignore.case = TRUE)) {
    tracktype <<- "predicted"
  } else if (grepl("predicted", selected_file, ignore.case = TRUE)) {
    tracktype <<- "predicted"
  } else {
    tracktype <<- NA
  }
...
trackmap <- ggplot() +
  ...
  geom_point(data = df_plot,
             aes(x=lon, y=lat, group = shark, 
                 fill = ifelse(g <= 0.75, "\u03B3t  ≤ 0.75", "\u03B3t  > 0.75"),
                 alpha = ifelse(g <= 0.75, 0.15, .975)),
             shape = 21,
             # alpha = 1, 
             size = 1.5) +  #original point size 1.15, new is 1.15
  
  # Add scale for movement persistence
  scale_fill_manual(name = "Behavioural index",
                    values = c("\u03B3t  ≤ 0.75" = "#1a1a1a",  # dark grey/black
                               "\u03B3t  > 0.75" = "orange"),
                    breaks = c("\u03B3t  ≤ 0.75", "\u03B3t  > 0.75"),
                    guide = guide_legend(override.aes = list(size = 3, alpha = 1))) +
  
  # Add alpha scale, i.e. here we want to suppress legend for alpha
  ...
  # basemap shapefile
  ggplot2::geom_sf(data = world, fill = "gray65", color = "black", size = .25, inherit.aes = F) + 
  ...
  theme(panel.grid = element_blank(), #legend.title = element_text(face = "bold"),
        ...
        ggtitle(sp.f); trackmap

## save your map
ggsave(file.path(saveloc, paste0("MPM_behavioural_index_map","_",sp_clean,".tiff")), width = 28, height = 21, units = "cm", dpi = 300)

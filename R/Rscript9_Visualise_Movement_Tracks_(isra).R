...
### Script:     ~2025_NWA_hammerhead_complex/R/Rscript9_Visualise_Movement_Tracks.R
### Author:     Vital Heim
### Version:    1.0
### ====================================================================================================
...
## Function to create custom colour ranges for plotting multiple individuals
...
## Output data
saveloc <- file.path("/",YOUR_IP, "Science","Projects_current", "2025_NWA_hammerhead_complex","Data_output", "Telemetry", "Maps_and_tracks")
### if saveloc does not yet exist, create it
if (!dir.exists(saveloc)) {
  dir.create(saveloc, recursive = TRUE)
  cat("Directory created:", saveloc, "\n")
} else {
  cat("Directory already exists:", saveloc, "\n")
}
...
## movement data
mov_tracks <- readRDS(file = file.path(dataloc, "Maps_and_tracks", "Data_aniMotum_CRW_output_segmented_rerouted_proj_WGS84_converted_with_coord_CIs_with_Argosfilter_data.rds")) |>
  dplyr::rename(
    shark = id
  ) %>%
  dplyr::mutate(
...
## save your map
ggsave(file.path(saveloc, "Movement_tracks_CTCRW_predicted_Sphyrna_all.tiff"), width = 21, height = 21, units = "cm", dpi = 300)
